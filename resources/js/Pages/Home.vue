      ...
      <div class="w-full md:w-1/2 h-full overflow-y-scroll">
        ...
        <div class="px-5 py-3 border-b-8 border-lighter flex">
          ...
          <form v-on:submit.prevent = "addNewTweet" class="w-full px-4 relative">
            <div>
                <textarea ref="tweet" placeholder="What's up?" class="mt-3 pb-6 w-full focus:outline-none" />
                <div class="flex items-center justify-between">
                    ...
                    <button type="submit" class="h-8 px-4 text-white bg-blue hover:bg-darklue focus:outline-none rounded-full">
                        Tweet
                    </button>
                </div>
            </div>
...
function addNewTweet () {
    if(tweet.value.value != ''){
        console.log(tweet.value.value)
        tweets.value.push({content: tweet.value.value})    
    }
}
</script>